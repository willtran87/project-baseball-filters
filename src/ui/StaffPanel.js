...
export class StaffPanel {
  constructor(container, state, eventBus, staffSystem) {
    ...
    this.eventBus.on('staff:assigned', () => this._rerender());
    this.eventBus.on('staff:unassigned', () => this._rerender());
  ...
  _renderHireTab() {
    ...
    if (candidates.length === 0) {
      return `
        ...
        <button data-action="refresh-candidates" style="${refreshStyle};padding:6px 12px;font-family:monospace;font-size:12px">${refreshLabel}</button>
      `;
    }
    ...
    let html = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        ...
        <button data-action="refresh-candidates" style="${refreshStyle};padding:5px 10px;font-family:monospace;font-size:12px">${refreshLabel}</button>
      </div>
    `;
    ...
    for (const c of candidates) {
      ...
      const traitHtml = c.trait
        ? `<span style="color:#cc44cc;font-size:11px;background:rgba(204,68,204,0.1);padding:1px 6px;border-radius:2px;margin-left:4px">${c.trait}</span>`
        : '';
      html += `
        <div style="display:flex;align-items:center;gap:8px;padding:8px 8px;margin-bottom:4px;background:rgba(255,255,255,0.03);border-left:3px solid ${canAfford ? '#00e436' : '#333'}">
          <div style="flex:1">
            ...
            ${c.traitDesc ? `<div style="color:#777;font-size:11px;font-style:italic;margin-top:2px">${c.traitDesc}</div>` : ''}
          ...
          <button data-action="hire" data-candidate-id="${c.id}"
            ...
            border:1px solid ${canAfford ? '#3a6a4a' : '#333'};padding:5px 10px;font-family:monospace;
            cursor:${canAfford ? 'pointer' : 'not-allowed'};font-size:12px;width:55px">
            HIRE
          </button>
        </div>
      `;
  ...
  _renderRosterTab() {
    ...
    let html = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        ...
        <button data-action="team-building" style="${tbStyle};padding:5px 10px;font-family:monospace;font-size:12px"
    ...
    for (const s of staff) {
      ...
      const traitTag = s.trait
        ? `<span style="color:#cc44cc;font-size:11px;margin-left:4px;background:rgba(204,68,204,0.1);padding:1px 6px;border-radius:2px">${s.trait}</span>`
        : '';
      ...
      const traitEffectHtml = traitEffect
        ? `<div style="color:#a78bfa;font-size:11px;margin-top:1px">${traitEffect.desc}</div>`
        : '';

      // Morale tier effect description
      const moraleTier = this.staffSystem.getMoraleTier(s);
      const moraleEffectHtml = moraleTier === 'high'
        ? '<div style="color:#00e436;font-size:11px;margin-top:1px">High Morale: +10% repair speed, +3 domain health</div>'
        : moraleTier === 'low'
          ? '<div style="color:#ff004d;font-size:11px;margin-top:1px">Low Morale: -15% repair speed, -3 domain health, quit risk</div>'
          : '';
        ...
        ? (() => {
            ...
            return `
              <div style="margin-top:2px">
                <span style="color:#ffec27;font-size:11px;background:rgba(255,236,39,0.15);padding:1px 6px;border-radius:2px">IN TRAINING (${t.daysRemaining}d left)</span>
      ...
      html += `
        <div data-action="select-staff" data-staff-id="${s.id}" style="display:flex;align-items:center;gap:8px;padding:8px 8px;margin-bottom:4px;background:rgba(255,255,255,0.03);border-left:3px solid ${isTraining ? '#ffec27' : '#29adff'};cursor:pointer">
          <div style="flex:1">
            <div style="color:#e0e0e0;margin-bottom:2px">
              <strong>${s.name}</strong>
              <span style="color:#ffec27;font-size:11px;margin-left:4px">Lv.${s.level}</span>
              ${specName ? `<span style="color:#c2c3c7;font-size:11px;margin-left:4px;background:rgba(255,255,255,0.08);padding:1px 6px;border-radius:2px">${specName}</span>` : ''}
              ${traitTag}
            </div>
            <div style="color:#888;font-size:12px">
          ...
          <div style="width:60px">
            <div style="font-size:11px;color:${moraleColor};margin-bottom:2px">MRL ${moralePct}%</div>
  ...
  _renderAssignTab() {
    ...
    let html = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        ...
        ${unassignedCount > 0 ? `
          <button data-action="auto-assign" style="background:#1a3a2a;color:#00e436;border:1px solid #3a6a4a;padding:5px 10px;font-family:monospace;cursor:pointer;font-size:12px">AUTO-ASSIGN</button>
        ` : ''}
      </div>
    `;
    ...
    for (const domain of domains) {
      ...
      html += `
        <div style="margin-bottom:10px;padding:8px 8px;background:rgba(255,255,255,0.02);border-left:3px solid ${domainColor}">
          <div style="color:${domainColor};margin-bottom:4px"><strong>${domainDef?.name ?? domain}</strong></div>
          ${assigned.length > 0
            ? assigned.map(s => `
              <div style="display:flex;align-items:center;gap:8px;padding:4px 0">
                <span style="color:#e0e0e0;flex:1">${s.name} <span style="color:#ffec27;font-size:11px">Lv.${s.level}</span></span>
                <button data-action="unassign" data-staff-id="${s.id}"
                  style="background:#3a2a2a;color:#ff8800;border:1px solid #5a3a3a;padding:4px 8px;font-family:monospace;cursor:pointer;font-size:11px">
                  Remove
                </button>
              </div>
            `).join('')
            : '<div style="color:#555;font-size:12px">No staff assigned</div>'
          }
        </div>
      `;
    }
    ...
    if (unassigned.length > 0) {
      ...
      for (const s of unassigned) {
        html += `
          <div style="display:flex;align-items:center;gap:8px;padding:6px 8px;margin-bottom:2px;background:rgba(255,255,255,0.02)">
            <span style="color:#e0e0e0;flex:1">${s.name} <span style="color:#ffec27;font-size:11px">Lv.${s.level}</span></span>
            ${domains.map(d => {
              ...
              return `<button data-action="assign" data-staff-id="${s.id}" data-domain="${d}"
                style="background:#1a2a3a;color:${dc};border:1px solid #3a4a5a;padding:4px 8px;font-family:monospace;cursor:pointer;font-size:11px">
                ${d}
              </button>`;
            }).join('')}
          </div>
  ...
  _renderStaffDetail() {
    ...
    let html = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <button data-action="back-to-roster" style="background:#2a2a3a;color:#888;border:1px solid #3a3a5a;padding:5px 8px;font-family:monospace;cursor:pointer;font-size:12px">&lt; Back</button>
      ...
      <div style="display:flex;gap:16px;margin-bottom:8px">
        <div>
          <div style="color:#888;font-size:11px">SPEED</div>
          <div style="color:#29adff;font-size:14px">${member.speed}</div>
        </div>
        <div>
          <div style="color:#888;font-size:11px">SKILL</div>
          <div style="color:#ffa300;font-size:14px">${member.skill}</div>
        </div>
        <div>
          <div style="color:#888;font-size:11px">MORALE</div>
          <div style="color:${moraleColor};font-size:14px">${moralePct}%</div>
        </div>
        <div>
          <div style="color:#888;font-size:11px">WAGE</div>
          <div style="color:#c2c3c7;font-size:14px">$${this.staffSystem.getEffectiveWage(member).effectiveWage}/d</div>
        </div>
      </div>

      <div style="margin-bottom:4px;color:#888;font-size:12px">Morale</div>
      <div style="background:#222;height:6px;margin-bottom:8px;border-radius:2px">
        <div style="background:${moraleColor};height:100%;width:${moralePct}%;border-radius:2px"></div>
      </div>

      <div style="margin-bottom:4px;color:#888;font-size:12px">Experience (${xpDisplay})</div>
      <div style="background:#222;height:6px;margin-bottom:8px;border-radius:2px">
        <div style="background:#ffec27;height:100%;width:${xpPct}%;border-radius:2px"></div>
      </div>

      ${specName
        ? `<div style="margin-bottom:8px">Specialization: <span style="color:#c2c3c7;background:rgba(255,255,255,0.08);padding:4px 8px;border-radius:2px">${specName}</span></div>`
        : ''
      }
      ...
      ${member.trait
        ? `<div style="margin-bottom:8px;padding:8px 8px;background:rgba(204,68,204,0.06);border-left:2px solid #cc44cc;border-radius:2px">
            <div style="color:#cc44cc;font-size:12px;margin-bottom:2px">Trait: ${member.trait}</div>
            <div style="color:#888;font-size:12px;font-style:italic">${member.traitDesc ?? ''}</div>
            ${this._getTraitEffectHtml(member)}
          </div>`
        : ''
      }
      ${member.backstory
        ? `<div style="margin-bottom:8px;padding:8px 8px;background:rgba(255,255,255,0.02);border-left:2px solid #555;border-radius:2px">
            <div style="color:#888;font-size:11px;margin-bottom:2px;text-transform:uppercase;letter-spacing:1px">Background</div>
            <div style="color:#aaa;font-size:12px;font-style:italic">"${member.backstory}"</div>
          </div>`
        : ''
      }
    `;
    ...
    if (moraleTier === 'high') {
      html += '<div style="margin-bottom:8px;padding:6px 8px;background:rgba(0,228,54,0.06);border-left:2px solid #00e436;border-radius:2px;color:#00e436;font-size:12px">High Morale: +10% repair speed, +3 domain health</div>';
    } else if (moraleTier === 'low') {
      html += '<div style="margin-bottom:8px;padding:6px 8px;background:rgba(255,0,77,0.06);border-left:2px solid #ff004d;border-radius:2px;color:#ff004d;font-size:12px">Low Morale: -15% repair speed, -3 domain health, quit risk</div>';
    }
    ...
    if (wageInfo.moraleMult !== 1.0 || wageInfo.specMult !== 1.0) {
      ...
      html += `<div style="margin-bottom:8px;padding:6px 8px;background:rgba(255,163,0,0.06);border-left:2px solid #ffa300;border-radius:2px;color:#ffa300;font-size:12px">Wage: $${wageInfo.baseWage} base x ${parts.join(', ')} = $${wageInfo.effectiveWage}/day</div>`;
    }
    ...
    if (isTraining) {
      ...
      const pct = Math.floor(((t.totalDays - t.daysRemaining) / t.totalDays) * 100);
      html += `
        <div style="margin-bottom:8px;padding:8px 8px;background:rgba(255,236,39,0.06);border-left:2px solid #ffec27;border-radius:2px">
          <div style="color:#ffec27;font-size:12px;margin-bottom:4px">IN TRAINING (${t.daysRemaining} day${t.daysRemaining !== 1 ? 's' : ''} remaining)</div>
          <div style="background:#222;height:6px;border-radius:2px">
            <div style="background:#ffec27;height:100%;width:${pct}%;border-radius:2px"></div>
          </div>
          <div style="color:#888;font-size:11px;margin-top:2px">Unavailable for work until training completes. +50 XP on completion.</div>
        </div>
      `;
    } else {
      ...
      html += `
        <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
          <button data-action="train-staff" data-staff-id="${member.id}"
            ...
            border:1px solid ${canTrain ? '#3a4a6a' : '#2a2a2a'};padding:5px 10px;font-family:monospace;
            cursor:${canTrain ? 'pointer' : 'not-allowed'};font-size:12px"
          ...
          ${!this.staffSystem.hasPraisedToday(member.id)
            ? `<button data-action="praise-staff" data-staff-id="${member.id}"
                style="background:#1a3a2a;color:#00e436;border:1px solid #3a6a4a;padding:5px 10px;font-family:monospace;cursor:pointer;font-size:12px"
                title="Praise this staff member (+5 morale, once/day)">
                Praise (+5 morale)
              </button>`
            : '<span style="color:#555;font-size:11px;padding:5px 0">Already praised today</span>'
          }
        </div>
      `;
    }
    ...
    // Specialize buttons
    if (canSpecialize) {
      html += '<div style="margin-bottom:8px;color:#aaa;font-size:12px">Choose Specialization:</div><div style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap">';
      for (const [key, spec] of Object.entries(SPECIALIZATIONS)) {
        html += `<button data-action="specialize" data-spec-key="${key}"
          style="background:#2a2a4a;color:#c2c3c7;border:1px solid #4a4a6a;padding:5px 8px;font-family:monospace;cursor:pointer;font-size:12px">
          ${spec.name}
        </button>`;
      }
      html += '</div>';
    ...
    html += `
      <div style="margin-top:12px;border-top:1px solid #3a3a5a;padding-top:8px">
        <button data-action="fire-staff" data-staff-id="${member.id}"
          style="background:#4a2a2a;color:#ff004d;border:1px solid #6a4a4a;padding:5px 10px;font-family:monospace;cursor:pointer;font-size:12px">
          Fire ${member.name}
        </button>
      </div>
    `;
  ...
  _getTraitEffectHtml(staffMember) {
    ...
    return `<div style="color:#a78bfa;font-size:11px;margin-top:2px">Effect: ${effect.desc}</div>`;
  }
    ...
    this._clickHandler = (e) => {
      ...
      switch (action) {
        ...
        case 'fire-staff': {
          ...
          const specName = member?.specialization ? SPECIALIZATIONS[member.specialization]?.name : null;
          const lossDetails = [
            member?.level > 1 ? `Level ${member.level}` : null,
            member?.xp > 0 ? `${member.xp} XP` : null,
            specName ? `${specName} specialization` : null,
            member?.morale > 50 ? 'morale bonuses' : null,
          ].filter(Boolean);
          const lossText = lossDetails.length > 0
            ? `<br><span style="color:#888;font-size:11px">You'll lose: ${lossDetails.join(', ')}.</span>`
            : '';
          showConfirmDialog(
            this.container,
            `Fire <strong style="color:#ff8800">${name}</strong>?${lossText}`,
            () => {
