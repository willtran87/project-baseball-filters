...
export class PrestigeMenu {
  ...
  _render() {
    ...
    if (activePerks.length === 0) {
      ...
      activePerksHtml = activePerks.map(p =>
        `<div style="display:flex;align-items:center;gap:6px;padding:3px 0;border-bottom:1px solid #222">
          <span style="color:#00e436;font-size:10px">&#10003;</span>
          <span style="color:#ffec27;font-size:10px">${p.name}</span>
          <span style="color:#888;font-size:8px;flex:1">${p.description}</span>
        </div>`
      ).join('');
    }
    ...
    if (availablePerks.length === 0) {
      ...
      unlocksHtml = availablePerks.map(p => {
        ...
        return `<div style="
          ...
          padding:6px 8px;
        ">
          <div style="font-size:10px;color:${canBuy ? '#c0c0d0' : '#666'};font-weight:bold;margin-bottom:2px">
            ${p.name}${tierLabel}
          </div>
          <div style="font-size:8px;color:#777;margin-bottom:4px;line-height:1.3">${p.description}</div>
          <button data-unlock-id="${p.id}" data-cost="${p.cost}" ${canBuy ? '' : 'disabled'} style="
            ...
            padding:2px 8px;font-family:monospace;font-size:8px;
    ...
    this._el.innerHTML = `
      ...
      ">
        <div>
          <span style="color:#ffec27;font-size:12px;letter-spacing:2px;font-weight:bold">LEGACY HALL</span>
          <span style="color:#8b4513;font-size:8px;margin-left:6px;letter-spacing:1px">PRESTIGE</span>
        </div>
      ...
      <div style="flex:1;overflow-y:auto;padding:10px 14px;">
        ...
        <div style="text-align:center;margin-bottom:14px;padding:8px;background:rgba(0,0,0,0.3);border-radius:3px;border:1px solid #333">
          <div style="font-size:8px;color:#8b4513;letter-spacing:2px;margin-bottom:4px">LEGACY POINTS</div>
        ...
        <div style="margin-bottom:14px">
          <div style="color:#8b4513;font-size:9px;letter-spacing:2px;margin-bottom:6px;border-bottom:1px solid #2a2a2a;padding-bottom:3px">
