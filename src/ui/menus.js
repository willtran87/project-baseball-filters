 ...
 \\   \\ '-' /
    ...
    inner.innerHTML = `
      ...
      <div style="display:flex;flex-direction:column;gap:8px;align-items:center">
        ...
        <button data-action="load-game" style="
          ...
          padding:9px 32px;font-family:monospace;font-size:14px;cursor:pointer;
      ...
      <div style="margin-top:20px;font-size:10px;color:#5f574f;line-height:1.6">
        Install filters on vent slots \u2022 Survive game days \u2022 Build reputation<br>
        <span style="color:#4a4a6a">Press S for Systems \u2022 SPACE to pause \u2022 ESC for menu</span>
      </div>
    `;
  ...
  _showDifficultySelect() {
    ...
    for (const d of difficulties) {
      ...
      buttonsHtml += `
        ...
        ">
          <div style="font-size:14px;color:${d.color};letter-spacing:2px;font-weight:bold;margin-bottom:2px">
            ${d.name}${isDefault ? ' <span style="font-size:10px;color:#888;letter-spacing:0">(default)</span>' : ''}
          </div>
          <div style="font-size:10px;color:#b0b0c0;margin-bottom:3px;font-style:italic">${d.desc}</div>
          <div style="font-size:10px;color:#888">${d.stats}</div>
        </button>`;
    }
    ...
    inner.innerHTML = `
      ...
      <div style="display:flex;flex-direction:column;gap:6px;align-items:center">
        <button data-action="resume" style="
          ...
          padding:9px 24px;font-family:monospace;font-size:14px;cursor:pointer;
        ...
        <button data-action="journal" style="
          ...
          padding:7px 24px;font-family:monospace;font-size:14px;cursor:pointer;
      ...
      <div style="margin-top:14px;font-size:10px;color:#5f574f">
  ...
  _showSaveSlots() {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      const ageHtml = age ? `<span style="color:#666;font-size:9px;margin-left:4px">${age}</span>` : '';
      slotsHtml += `
        <button data-save-slot="${i}" style="
          ...
          margin-bottom:6px;
        ">
          <div style="font-size:12px;color:#29adff;letter-spacing:1px;margin-bottom:2px">
            SLOT ${i}${ageHtml}
          </div>
  ...
  _showLoadSlots() {
    ...
    // Auto-save slot
    const autoAgeHtml = autoAge ? `<span style="color:#666;font-size:9px;margin-left:4px">${autoAge}</span>` : '';
    ...
    slotsHtml += `
      ...
      ">
        <div style="font-size:12px;color:#00e436;letter-spacing:1px;margin-bottom:2px">
          AUTO-SAVE${autoAgeHtml}
        </div>
        <div style="font-size:11px;color:${autoEmpty ? '#555' : '#c0c0d0'}">${autoLabel}</div>
      </button>`;
  ...
  _renderGameOver() {
    ...
    if (isWin && sandboxGoals && sandboxGoals.length > 0) {
      ...
      const sgItems = sandboxGoals.map(g =>
        `<span style="color:${g.completed ? '#00e436' : '#555'};font-size:10px">${g.completed ? '\u2713' : '\u25cb'} ${g.name}</span>`
      ).join('<br>');
      sandboxHtml = `
        <div style="margin-top:6px;padding-top:4px;border-top:1px solid #333">
          <div style="color:#ff77a8;font-size:10px;letter-spacing:1px;margin-bottom:3px">SANDBOX CHALLENGES: ${sgDone}/${sandboxGoals.length}</div>
          ${sgItems}
        </div>
      `;
    }
    ...
    if (recentAchievements.length > 0) {
      const items = recentAchievements.map(id => {
        ...
        return `<span style="color:#ffec27;font-size:10px">\u2605 ${label}</span>`;
      }).join('<br>');
      recentHtml = `
        <div style="margin-top:6px;padding-top:4px;border-top:1px solid #333">
    ...
    inner.innerHTML = `
      ...
      <div style="font-size:12px;color:#83769c;margin-bottom:12px;letter-spacing:1px">
        ${subtitle}
      </div>
      <div style="font-size:11px;color:#b0b0c0;margin-bottom:16px;line-height:1.6;
        max-width:380px;margin-left:auto;margin-right:auto;font-style:italic;
        border-left:2px solid ${titleColor};padding-left:12px;text-align:left">
        ${narrative}
      </div>
      <div style="font-size:11px;color:#888;margin-bottom:16px;background:rgba(0,0,0,0.3);padding:8px 12px;border-radius:3px;border:1px solid #333">
        ...
        ${data.prestigeBreakdown ? `
          <div style="margin-top:6px;padding-top:4px;border-top:1px solid #333">
            <div style="color:#ffec27;font-size:10px;letter-spacing:1px;margin-bottom:3px">LEGACY POINTS EARNED: +${data.prestigeBreakdown.total}</div>
      ...
      <div style="display:flex;flex-direction:column;gap:6px;align-items:center">
        ${isWin ? `<button data-action="credits" style="
          ...
          padding:8px 32px;font-family:monospace;font-size:16px;cursor:pointer;
          letter-spacing:2px;
        ">CREDITS</button>` : ''}
    ...
    const renderShop = () => {
      ...
      for (const def of defs) {
        ...
        gridHtml += `
          ...
          ">
            <div style="font-size:12px;color:${def.unlocked ? '#ffec27' : '#c0c0d0'};letter-spacing:1px;margin-bottom:2px;font-weight:bold">
              ${def.name}
            </div>
            <div style="font-size:10px;color:#888;margin-bottom:4px;line-height:1.4">${def.description}</div>
            <button data-buy-unlock="${def.id}" data-cost="${def.cost}" ${canBuy ? '' : 'disabled'} style="
              ...
              padding:5px 12px;font-family:monospace;font-size:10px;
  ...
  _showCredits() {
    ...
    content.innerHTML = `
      ...
      <div style="font-size:11px;color:#888;letter-spacing:2px;margin-bottom:4px">CREATED WITH</div>
      <div style="font-size:14px;color:#29adff;margin-bottom:20px;letter-spacing:1px">Claude Code</div>

      <div style="font-size:11px;color:#888;letter-spacing:2px;margin-bottom:8px">YOUR LEGACY</div>
      <div style="font-size:12px;color:#c0c0d0;line-height:2;margin-bottom:20px">
      ...
      <div style="font-size:12px;color:#ffa300;line-height:1.8;margin-bottom:20px;font-style:italic;max-width:340px;margin-left:auto;margin-right:auto">
        ...
        <span style="color:#888;font-size:11px">-- Hank "Duct Tape" Doolan</span>
      </div>

      <div style="font-size:14px;color:#ffec27;margin-bottom:20px;letter-spacing:2px">
        Thank you for playing!
      </div>

      <div style="font-size:10px;color:#555;margin-top:16px">Press any key to continue</div>
    `;
